...
namespace 仓库管理
{
    public partial class 出库修改窗口 : Form
    {
        DataTable org_dt;
        public 出库修改窗口()
        {
        ...
        private void 出库修改窗口_Load(object sender, EventArgs e)
        {
            initForm();
        }
        ...
        private void 出库人textBox_Enter(object sender, EventArgs e)
        {
        ...
        private void listBox3_SelectedValueChanged(object sender, EventArgs e)
        {
            ...
            出库人textBox.Text = string.Join(",", list1);
            label8.Text = string.Join(",", list2);
        }

        private void 复位Button_Click(object sender, EventArgs e)
        {
            仓库textBox.Text = 备件textBox.Text = 出库人textBox.Text = "";
            出库开始时间textBox.Text = 出库结束时间textBox.Text = "";
        ...
        private void 开始查询()
        {
            ...
            string 出库用户ID = label8.Text;
            if (string.Compare(出库开始时间textBox.Text, 出库结束时间textBox.Text) == 1)
            {
                出库开始时间textBox.Text = 出库结束时间textBox.Text;
            }
            string 出库开始时间 = 出库开始时间textBox.Text;
            string 出库结束时间 = 出库结束时间textBox.Text;
            string 时间范围 = "";
            if (出库开始时间 != "")
            {
                时间范围 += " and B.操作时间 >= '" + 出库开始时间 + "'";
            }
            if (出库结束时间 != "")
            {
                时间范围 += " and (B.操作时间 <= '" + 出库结束时间 + "' or B.操作时间 like '" + 出库结束时间 + "%')";
            }
            string 去处= 去处textBox.Text;
            if (去处!= "")
            {
                去处 = " and 去处 like '%"+去处+"%'";
            }
            string sqlText = string.Format("select A.出库ID,D.仓库名称,E.备件名称,F.用户名,A.数量,B.操作时间,A.备注 " +
            "from dbo.出库表 A " +
            "inner join dbo.出库历史表 B on B.出库ID = A.出库ID " +
            ...
            "where D.仓库ID in ({0}) and A.备件ID in ({1}) and B.操作用户ID in ({2}) and A.删除标志=0 and B.操作='添加' and B.字段='出库ID' {3} {4}", 仓库ID, 备件ID, 出库用户ID, 时间范围,去处);
        ...
        private void 保存修改button_Click(object sender, EventArgs e)
        {
            if (dataGridView1.DataSource != null)
            {
                ...
                SqlHelper sh = new SqlHelper();
                int? result = sh.IUDbyDataTableReturnInt("select 出库ID,数量,备注 from dbo.出库表", (DataTable)(dataGridView1.DataSource), null, CommandType.Text);
                string org_value, mod_value, 出库ID;
                bool IDTitle;
                for (int i = 0; i < dataGridView1.Rows.Count; i++)
                {
                    ...
                    for (int j = 2; j < dataGridView1.ColumnCount; j++)
                    {
                        ...
                        {
                            出库ID = Convert.ToString(org_dt.Rows[i].Field<int>("出库ID"));
                            if (IDTitle)
                            {
                                list.Add("  出库ID：" + 出库ID);
                                IDTitle = false;
                            ...
                            plist.Add(new Tuple<string, string, string>(出库ID, org_dt.Columns[j].ColumnName, mod_value));
                        }
                    }
                }
                if (list.Count > 0)
        ...
        private void dataGridView1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            string 出库ID = Convert.ToString(dc[0].Value);
            if (MessageBox.Show("是否删除“出库ID：" + 出库ID + "”?", "删除记录", MessageBoxButtons.OKCancel) == DialogResult.OK)
            {
                SqlHelper sh = new SqlHelper();
                string sqlText = string.Format("update dbo.出库表 set 删除标志=1 where 出库ID='{0}' and 删除标志=0", 出库ID);
                int? result = sh.IUDReturnInt(sqlText, null, CommandType.Text);
                if (result > 0)
                {
                    MessageBox.Show("删除“出库ID：" + 出库ID + "”成功");

                    string 操作用户ID = Form1.loginUser;
                    string 操作 = "删除";
                    string 操作时间 = DateTime.Now.ToString("yyyyMMddHHmmss");
                    string 详细内容 = "出库ID：" + 出库ID;
                    string 备注 = "";
                    sqlText = string.Format("insert into dbo.出库历史表(出库ID,操作用户ID,操作,操作时间,备注)values('{0}','{1}','{2}','{3}','{4}')", 出库ID, 操作用户ID, 操作, 操作时间, 备注);
                    result = sh.IUDReturnInt(sqlText, null, CommandType.Text);
