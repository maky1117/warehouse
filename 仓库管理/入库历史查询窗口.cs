...
namespace 仓库管理
{
    public partial class 入库历史查询窗口 : Form
    {
        public 入库历史查询窗口()
        {
            InitializeComponent();
        ...
        private void 入库历史查询窗口_Load(object sender, EventArgs e)
        {
        ...
        private void initForm()
        {
            ...
            string sqlText = "select distinct 操作 from dbo.入库历史表";
        ...
        private void 查询button_Click(object sender, EventArgs e)
        {
            ...
            string sqlText = string.Format("select A.历史ID,A.操作时间,B.用户名,A.操作,A.入库ID,A.字段,A.值,A.备注 " +
            "from dbo.入库历史表 A inner join dbo.用户表 B on A.操作用户ID = B.用户ID " +
            "where 操作 in ('{0}') and 操作用户ID in ({1}) {2}", 操作.Replace(",","','"), 操作用户ID, 时间范围);
            //string sqlText = string.Format(@"select 入库ID,A.仓库ID,A.备件ID,A.入库用户ID,A.数量,A.入库时间,A.备注 from (select 入库ID,仓库ID,备件ID,入库用户ID,数量,入库时间,备注 
            //from dbo.入库表 where 仓库ID in ({0}) and 备件ID in ({1}) and 入库用户ID in ({2}) and 入库时间 between '{3}' and '{4}')A
            //inner join dbo.仓库表 B on A.仓库ID = B.仓库ID
            //inner join dbo.备件表 C on A.备件ID = C.备件ID
            //inner join dbo.用户表 D on A.入库用户ID = D.用户ID", 仓库ID, 备件ID, 入库用户ID, 入库开始时间, 入库结束时间);
            DataTable dt = new DataTable();
            dt = sh.selectReturnDataSet(sqlText, null, CommandType.Text).Tables[0];
